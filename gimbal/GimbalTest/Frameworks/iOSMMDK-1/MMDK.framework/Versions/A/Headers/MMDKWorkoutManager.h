...
/**
 MMDKWorkout is the object that represents a workout
 */
@interface MMDKWorkoutManager : NSObject

+ (MMDKWorkoutManager *)instance;
...
/**
 Method to get the name for a given activityTypeID
 */
- (NSArray *)recentActivityTypes;

/**
 Method to provide a mock workout to save
 */
- (MMDKWorkout *)getMockWorkout;

/**
 Class method to provide a MMDKActivityType for a given activityTypeID
 @param actID is the activityTypeID for the desired MMDKActvityType
 */

+ (MMDKActivityType *)activityTypeForID:(NSUInteger)actID;

- (NSArray *)getLocalWorkouts;

- (MMDKWorkoutDO *)getLocalWorkoutWithKey:(NSNumber *)key;
@end
