...
/**
 * Method triggered when estimote beacon receive content assigned in the cloud
 *
 * @param manager estimote beacon manager
 * @param updated beacon
 * @param error information
 *
 * @return void
 */
-(void)beaconManager:(ESTBeaconManager *)manager
didReceivedContentForBeacon:(ESTBeacon *)beacon
           withError:(NSError*)error;
...
/**
 * Setup API key generated with the cloud
 *
 * @param region estimote beacon region
 *
 * @return void
 */
+(void)setupAPIKey:(NSString*)apiKey andAppID:(NSString*)appID;
...
/// @name Estimote CMS API Handling methods
#pragma mark - Estimote CMS API Handling methods

/**
 * Validate if user is logged in to Estimote CMS
 *
 * @return YES or NO
 */
+(BOOL)isLoggedIn;
...
/**
 * Login into Estimote CMS to get acces to API methods.
 *
 * @property email Estimote CMS email address of the user
 * @property password Estimote CMS password
 *
 * @return void
 */
 ...
 * Pre-fetch beacons that are assign to your account.
 ...
 * @return void
 */
-(void)fetchEstimoteBeaconsWithContent:(BOOL)content completion:(ESTArrayCompletionBlock)completion;
